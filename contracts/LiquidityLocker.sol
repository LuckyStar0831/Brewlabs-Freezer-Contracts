...
contract LiquidityLocker is Ownable, ReentrancyGuard {
  ...
  struct FeeStruct {
    ...
    uint256 extensionDiscount; // fee discount since second lock
    
    IERCBurn secondaryFeeToken; // Official Token
  ...
  constructor(IUniFactory _uniswapFactory) {
    ...
    gFees.ethFee = 5e17; // 0.5 ether
    gFees.extensionDiscount = 900; // 90%
  ...
  function setSecondaryFeeToken(address _secondaryFeeToken) external onlyOwner {
   ...
   */
  function setReferralTokenAndHold(IERCBurn _referralToken, uint256 _hold) external onlyOwner {
    gFees.referralToken = _referralToken;
    gFees.referralHold = _hold;
  }
  
  function setFees(uint256 _referralPercent, uint256 _referralDiscount, uint256 _ethFee, uint256 _extensionDiscount, uint256 _secondaryTokenFee, uint256 _secondaryTokenDiscount, uint256 _liquidityFee) external onlyOwner {
    gFees.referralPercent = _referralPercent;
    gFees.referralDiscount = _referralDiscount;
    gFees.ethFee = _ethFee;
    gFees.extensionDiscount = _extensionDiscount;
  ...
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _referral, bool _fee_in_eth, address payable _withdrawer) external payable nonReentrant {
    ...
    if (!feeWhitelist.contains(msg.sender)) {
      if (_fee_in_eth) { // charge fee in eth
        ...
        if(users[msg.sender].used[_lpToken]) {
          ethFee = ethFee.mul(1000 - gFees.extensionDiscount).div(1000);
        }
    ...
    if(!user.used[_lpToken]) {
      user.used[_lpToken] = true;
    }
  ...
  /**
   * @notice emergencyUnlock a specified amount from a lock. _index and _lockID ensure the correct lock is changed
   * this prevents errors when a user performs multiple tx per block possibly with varying gas prices
   */
  function emergencyUnlock (address _lpToken, uint256 _index, uint256 _lockID, address _owner) external onlyOwner nonReentrant {
  ...
  function splitLock (address _lpToken, uint256 _index, uint256 _lockID, uint256 _amount) external payable nonReentrant {
    ...
    uint256 ethFee = gFees.ethFee.mul(1000 - gFees.extensionDiscount).div(1000);
    require(msg.value == ethFee, 'FEE NOT MET');
